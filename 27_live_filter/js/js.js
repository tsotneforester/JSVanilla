...
let nodeForHTML = ""; // html tags with info to be sent in index.html
let array = Object.keys(data[0]); // creates array with table headers
let numberOfColumns = Object.keys(data[0]).length;
// ---------------- Generate  <TH> ---------------------
nodeForHTML += "<tr>";
for (let i = 0; i < numberOfColumns; i++) {
  nodeForHTML += `<th>${array[i]}</th>`;
}
nodeForHTML += "</tr>";
...
for (let i = 0, n = data.length; i < n; i++) {
  ...
  for (let ii = 0; ii < numberOfColumns; ii++) {
    nodeForHTML += `<td>${array[ii]}</td>`;
  }
  nodeForHTML += "</tr>";
}

table.innerHTML = nodeForHTML;
const trlength = table.rows.length;
//|||||||||||||||| Filter Functional |||||||||||||||||||||
...
inputs.forEach((input, ind) => {
  input.addEventListener("input", function () {
    ...
    for (let i = 2; i < trlength; i++) {
      ...
      for (let ii = 0; ii < numberOfColumns; ii++) {
        if (tableRow.getElementsByTagName("td")[ii].innerHTML.toLowerCase().indexOf(inputs[ii].value) == -1) {
...
button.addEventListener("click", exportExcelFile);
//|||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||
